...
def run_it(config,output_folder, i, file, bins_Mf,params_template):
    ...
    if not os.path.exists(output_folder+'/targets/targets_junk/targets_{0}'.format(i)+'.pkl'):
            ...
            # add 2 more noisetiers with all the other targets not passing the covariance cut. 
            for ix in range(len(tab_mute.images)):
                tab_mute.images[ix].flags = copy.deepcopy(flag_s[ix])
...
def make_targets(output_folder,**config):
    ...
    # add -1 bin:
    if (list_1['info'][config['steps']]['n'])>0:
        prihdu,tbhdu = save(list_1['list'][binx],list_1['info'][config['steps']],-1)
        tbhdu_list.append(tbhdu)
        
    for binx in range(config['steps']):
        if (list_1['info'][binx]['n'])>0:
                prihdu,tbhdu = save(list_1['list'][binx],list_1['info'][binx],binx)
                tbhdu_list.append(tbhdu)
                
    # add last bin.
    if (list_1['info'][config['steps']+1]['n'])>0:
        prihdu,tbhdu = save(list_1['list'][binx],list_1['info'][config['steps']+1],config['steps'])
        tbhdu_list.append(tbhdu)
...
config['steps']
    ...
    # convert to Gary's format **************
    
    
    ff_in = pf.open(output_folder+'/target_sample.fits')
    ntiers = len(ff_in)-1
    hdulist = pf.HDUList([pf.PrimaryHDU()])

    # First we're going to create a table the concatenates all the current tables
    nrows = np.sum([ff_in[i].data.shape[0] for i in range(1,ntiers+1)])
    hdu = pf.BinTableHDU.from_columns(ff_in[1].columns, nrows=nrows)
    for key in ('WT_N','WT_SIG'):
        hdu.header[key] = ff_in[1].header[key]
    for cname in ff_in[1].columns.names:
        sofar = 0
        for i in range(1,ntiers+1):
            nn = ff_in[i].data.shape[0]
            hdu.data[cname][sofar:sofar+nn] = ff_in[i].data[cname]
            sofar += nn  
    hdulist.append(hdu)
    del hdu
    ...
    for tier in range(1,ntiers+1):
        hdr = ff_in[tier].header
        tab = ff_in[tier].data
        # Build covariance matrix for even parity
        cov_mean = np.mean(tab['cov_even'],axis=0)
        n = 5
        data = np.zeros((n,n))
        index = 0
        for i in range(n):
            for j in range(i,n):
                data[i,j] = cov_mean[index]
                data[j,i] = cov_mean[index]
                index += 1
        hdu = pf.ImageHDU(data)
        for key in ('WT_N','WT_SIG','TIERNAME'):
            hdu.header[key] = hdr[key]
        # Record mean covariance of odd moments in header
        cov_mean = np.mean(tab['cov_odd'],axis=0)
        hdu.header['COVMXMX'] = cov_mean[0]
        hdu.header['COVMXMY'] = cov_mean[1]
        hdu.header['COVMYMY'] = cov_mean[2]
        hdulist.append(hdu)
        del hdu
    ...
    # Save the output
    if astropy_version >= 1.3:
            hdulist.writeto(output_folder+'/target_sample.fits',overwrite=True)
    else:
            hdulist.writeto(output_folder+'/target_sample.fits',clobber=True)

    #pf.info(output_folder+'/target_sample.fits')
