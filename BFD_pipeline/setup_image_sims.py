...
def setup_image_sims(output_folder, **config):
    """
    Set up image simulations for the specified output folder and configuration.
    ...
    Notes:
        - This function assumes the availability of certain dependencies and modules.

        - This function is designed to work with a specific stage in the image simulation pipeline ('save_galaxy_params').
          It reads PSF and galaxy models from deep fields MOF fits and saves the parameters into pickle files.
          These files are then ready to be used to generate galaxies at the next stage.
    ...
    if config['stage'] == 'save_galaxy_params':
        ...
        while run_count < runs:
            if run_count + comm.rank < runs:
                ...
                for jj, pos_u in enumerate(pos):
                    if ((jj >= min_c) & (jj < max_c)):
                        ...
                        try:
                            ...
                            indexes_band = [MOF_deep_field.return_band_val(band, indtostr=False)
                ...
                # here save the chunk
                np.save(output_folder + '/gal_psf_params/chunk_' + str(run_count + comm.rank) + '.npy',
                        gal_psf_dict)
            run_count += comm.size
