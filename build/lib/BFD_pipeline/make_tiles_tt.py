...
def pipeline_targets(config, params_image_sims, ii_chunk, do_templates = False):
    ...
    if not os.path.exists(path):
        ...
        if ((not config['grid_targets']) & (config['poisson']) & (config['mode_detection'] =='detection')):
            ...
            STAMP_SIM = True
        if not do_templates:            
            save_(tab_targets,config['output_folder']+'/targets/'+'ISp_targets_{0}.fits'.format(ii_chunk))
            save_(tab_targets_m,config['output_folder']+'/targets/'+'ISm_targets_{0}.fits'.format(ii_chunk))
