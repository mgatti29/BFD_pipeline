            ...
            for replica in range(number_of_replicas):
                ...
                if config['setup_image_sims']:
                    ...
                    if replica%2 == 0:
                        ...
                        redoit = True
                        while redoit:
                            sim_p,sim_m,sim_PSF,p0,p0_PSF,fluxes,jac = tab_detections.generate_simulated_images(index,config,params_image_sims,use_COADD_only=config['COADD_only'],noiseless=config['noiseless'],  maskless = config['maskless'],size_stamp='auto',index_P0= index_run,index_P0_PSF=config['index_P0_PSF'],noise_factor = config['noise_factor'],count_replica=count_replica ,resize_sn=resize_sn, noise_ext = noise_ext,g1=[0.,0.],g2=[0.,0.])
                            count_replica+=1
