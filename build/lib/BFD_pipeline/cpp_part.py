                    ...
                    if 'selection' in todos:
                        while run_count<len(noise_tiers):
                            ...
                            if (run_count+comm.rank) < len(noise_tiers):
                                ...
                                a.write('useTier {0}\n'.format(noise_tiers[run_count+comm.rank]))
                                a.write('chunk 10000\n')
                                a.close()
                                os.system('/global/project/projectdirs/des/BFD_Y6/bfd/bin/calculateSelection config_{0}.config -selectSN {1},{2}  -useTier {0}'.format(noise_tiers[run_count+comm.rank],config['minsn'],config['maxsn']))
                                os.remove('config_{0}.config'.format(noise_tiers[run_count+comm.rank]))
                                run_count+=1
                ...
                if 'split' in todos:
                    ...
                    while run_count<chunks:
                        ...
                        if run_count+comm.rank<chunks:
                            ...
                            targets_i[1] = hdu
                            targets_i[1].header['WT_N'] = targets[1].header['WT_N']
                            targets_i[1].header['WT_SIG'] = targets[1].header['WT_SIG']
                            ...
                            for tier in range(2,len(targets)):
                                ...
                                hdu.header['WT_N'] =   targets[0].header['WT_N']
                                hdu.header['WT_SIG'] = targets[0].header['WT_SIG']
                    ...
                    while run_count<chunks:
                        ...
                        if run_count+comm.rank<chunks:
                            for u in range(len(noise_tiers)):
                                ...
                                target = output_folder+'/{0}targets_sample_g_{1}.fits'.format(add,run_count+comm.rank)
                                ma = pf.open(target)
                                if 1==1:
                                #try:
                                #    ma[1].data['NUNIQUE']
                                #except:
                                    ...
                                    target = output_folder+'/{0}targets_sample_g_{1}.fits'.format(add,run_count+comm.rank)
                                    a = open('config_{0}_{1}.config'.format(run_count+comm.rank,u),'w')
                                    a.write('templateFile {0} \n'.format(path_templates+'/templates_NOISETIER_{0}.fits'.format(noise_tiers[u])))
                                    a.write('targetFile {0} \n'.format(target))
                                    a.write('useTier {0}\n'.format(noise_tiers[u]))
                                    a.write('nSample 30000 \n'.format(noise_tiers[u]))
                                    #nSample     = 30000   
                                    a.write('chunk {0}\n'.format(config['number_targets_chunk']))
                                    a.close()


                                    os.system('/global/project/projectdirs/des/BFD_Y6/bfd/bin/tableIntegrate config_{0}_{1}.config  -selectSN {3},{4} -useTier {2}'.format(run_count+comm.rank,u,noise_tiers[u],config['minsn'],config['maxsn']))
                                    os.remove('config_{0}_{1}.config'.format(run_count+comm.rank,u))
